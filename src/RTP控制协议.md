...
RTP协议包含两部分：*数据传输协议*和*控制协议*, 我们分别在本章和上一章说明。
控制协议 RTCP，用于定期报告数据包接收质量、参与者标识、源的描述信息、会话成员变化以及同步媒体流必要信息。
...
RTCP包类型不只是一种类型，本章标题为《RTCP数据包格式》的小节介绍了五种标准数据包类型，以及汇总到聚合数据包的规则，聚合的目的是方便传输。《数据包校验》部分介绍RTCP数据包的校验算法。

根据本章后面**RTCP时序规则**中的规则，定期发送聚合数据包。数据包之间的间隔我们称之为*传输间隔*。所有RTCP活动都发生在整数倍的间隔时间内。除了作为包间隔的时间外，它还是计算接收质量统计信息的时间，以及更新源描述和音视频同步的时间。时间间隔会根据所使用的媒体格式和会话的大小而有所不同。通常，小型会话连接的间隔时间大约在5秒，但是大型的会话会增加到几分钟。在计算报告发送间隔时优先从发送端考虑，因此发送端的发送源信息和音视频同步信息发送得比较频繁。而接受者的上报通常较少。
...
每一个RTP的会话都是由一个网络地址和一对端口号做为标识：一个端口用于RTP数据，另一个端口用于RTCP数据。其中RTP数据端口应为偶数，RTCP端口应该是RTP端口加 1，例如，如果媒体数据正在使用 UDP 5004 端口，那么控制信道将在与RTP相同的IP地址上的端口 5005 上。
...
- 结束（BYE）
- 应用定义（APP
它们遵循统一的格式（图5.1），其中Format-specific信息，每种类型都不同。
